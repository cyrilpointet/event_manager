<template>
    <div>
        ...
        <div v-else>
            ...
            <v-card class="mb-4">
                <v-card-title>Membres invités</v-card-title>
                <v-card-text>
...
import HappeningMembers from "@/happening/component/HappeningMembers";
import HappeningMembersManager from "@/happening/component/HappeningMembersManager";

export default {
    name: "HappeningPage",
    components: { UpsertHappening, HappeningMembers, HappeningMembersManager },
    data: () => {
        return {
            edit: false,
        };
    ...
    methods: {
        ...
        async deleteHappening() {
            try {
                this.$store.commit("user/removeHappening", this.happening.id);
                await this.$store.dispatch(
